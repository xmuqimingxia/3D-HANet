...
class AnchorHeadSingleV2(AnchorHeadTemplate):
    ...
    def forward(self, data_dict):
        ...
        if self.training:
            ...
            num_box_1 = gt_boxes_1.size(1)
            point_indices_1 = point_indices_1.reshape(-1)
            for i in range(num_box_1):
                mask = point_indices_1 == i
                points_hm_1[:, mask] = self.gauss_fun(points_mean_1[0,mask,:], gt_boxes_1[0,i,:])
            targets_dict['gt_hm_1'] = points_hm_1

            # np.save('points_hm_1.npy', points_hm_1.cpu().numpy())
            # np.save('points_mean_1.npy', points_mean_1.cpu().numpy())
            # exit()
            ...
            # point_indices_1 = point_indices_1.reshape(-1)
            # mask = point_indices_1 != -1
            # forground = points_mean_1[:,mask]
            # forground_point_indices_1 = point_indices_1[mask]
            # forground_Num_points = forground.size(1)
            # for i in range(forground_Num_points):
            #     ind = forground_point_indices_1[i]
            #     if ind != -1:
            ...
            num_box_2 = gt_boxes_2.size(1)
            point_indices_2 = point_indices_2.reshape(-1)
            for i in range(num_box_2):
                mask = point_indices_2 == i
                points_hm_2[:, mask] = self.gauss_fun(points_mean_2[0,mask,:], gt_boxes_2[0,i,:])
            targets_dict['gt_hm_2'] = points_hm_2
